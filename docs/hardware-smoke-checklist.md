...
- [ ] Open a published UART or RS-485 CSV in Files, compare timestamps, direction, byte counts, hex and ASCII views with raw rows, filter RX/TX, page through more than eight rows, and return to the same folder. Confirm `.TMP` remains text preview.
  - ST7121, 2026-09-22: a verified COM7 flash of the development build opened a newly published UART log with ten one-byte TX rows. The viewer displayed `61` in Hex and `a` in ASCII, matched the terminal's seconds and TX byte counts, paged from rows 1-8 to 9-10, showed no rows under RX, restored ten rows under TX, and returned to `/sdcard/UART/260922`. Raw CSV inspection, an RX row, a retained `.TMP`, RS-485, and the second panel family remain open.
...
- [ ] A valid OTA image boots, passes its health window, and preserves settings and SD data.
  - ST7121, 2026-09-22: the published v0.5.1 app booted from the tablet's pre-existing factory/4 MiB OTA partition table with Wi-Fi, internal SPIFFS, and SD mounted. Its Install latest button downloaded and booted the published v0.6.0 app in OTA slot 0. The tablet's older bootloader left the image in `new` state; after backing up and replacing only the bootloader region with the v0.6.0 release bootloader, a reboot and health window reached `validated`, System recorded `Installed v0.6.0-20816206`, and Wi-Fi, internal SPIFFS, and SD stayed mounted. A current-layout clean install and the ILI9881C family remain open.
